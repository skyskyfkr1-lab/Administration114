...
const Contact: React.FC = () => {
  ...
  return (
    <div className="pb-16 md:pb-24">
      ...
      <div className="container mx-auto px-4 md:px-6 mt-10 md:mt-20 max-w-5xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-20 items-start">
          {/* Quick Contact Buttons */}
          <div className="space-y-6 md:space-y-10 px-2 md:px-0">
            <h2 className="text-lg md:text-4xl font-black mb-6 md:mb-12 break-keep tracking-tighter">빠른 문의 방법</h2>
            
            <a href={`tel:${CONTACT.phone.replace(/-/g, '')}`} className="flex items-center gap-4 md:gap-8 p-5 md:p-8 bg-white rounded-2xl md:rounded-3xl shadow-lg border border-gray-50 active:scale-[0.98] transition-all group overflow-hidden">
              <div className="bg-[#F5A623] p-3 md:p-6 rounded-xl md:rounded-2xl text-white shadow-lg shadow-[#F5A623]/20 shrink-0">
                <Smartphone className="w-5 h-5 md:w-8 md:h-8" />
              </div>
              <div className="flex-grow min-w-0">
                <p className="text-[9px] md:text-xs text-gray-400 font-bold uppercase tracking-widest mb-0.5 md:mb-1">휴대폰 직통 상담</p>
                <p className="font-black text-base md:text-3xl text-[#002C5F] tabular-nums truncate leading-tight">
                  {CONTACT.phone}
                </p>
              </div>
              <ChevronRight className="text-gray-300 shrink-0 w-4 h-4 md:w-5 md:h-5" />
            </a>

            <a href={`tel:${CONTACT.office.replace(/[()-]/g, '')}`} className="flex items-center gap-4 md:gap-8 p-5 md:p-8 bg-white rounded-2xl md:rounded-3xl shadow-lg border border-gray-50 active:scale-[0.98] transition-all group overflow-hidden">
              <div className="bg-[#002C5F] p-3 md:p-6 rounded-xl md:rounded-2xl text-white shadow-lg shadow-[#002C5F]/20 shrink-0">
                <Phone className="w-5 h-5 md:w-8 md:h-8" />
              </div>
              <div className="flex-grow min-w-0">
                <p className="text-[9px] md:text-xs text-gray-400 font-bold uppercase tracking-widest mb-0.5 md:mb-1">사무실 대표 번호</p>
                <p className="font-black text-base md:text-3xl text-[#002C5F] tabular-nums truncate leading-tight">
                  {CONTACT.office}
                </p>
              </div>
            ...
            <div className="bg-gray-50 p-5 md:p-6 rounded-xl md:rounded-2xl border border-gray-100">
              <p className="text-[10px] md:text-xs font-bold text-[#002C5F] mb-1.5 md:mb-2 uppercase tracking-wide">운영 시간</p>
              <p className="text-xs md:text-base font-medium text-gray-600 leading-relaxed break-keep">
          ...
          {/* Map Section */}
          <div className="space-y-6 md:space-y-10 px-2 md:px-0">
            <h2 className="text-lg md:text-4xl font-black mb-6 md:mb-12 break-keep tracking-tighter">찾아오시는 길</h2>
