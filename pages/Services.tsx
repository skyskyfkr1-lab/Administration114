  ...
  return (
    <div className="pb-24">
      ...
      <div className="container mx-auto px-4 mt-12 md:mt-20">
          ...
          {SERVICES.map((service, idx) => (
            <div key={service.id} className="bg-white rounded-2xl overflow-hidden shadow-sm flex flex-col md:flex-row border border-gray-100 hover:border-[#F5A623] transition-colors">
              <div className="md:w-1/3 bg-gray-50 flex items-center justify-center p-6 md:p-8">
                <div className="text-[#0F4C81] scale-125 md:scale-150">{service.icon}</div>
              </div>
              <div className="md:w-2/3 p-6 md:p-8">
                <h3 className="text-xl md:text-2xl font-bold mb-3 md:mb-4 break-keep">{service.title}</h3>
                <div className="text-gray-600 mb-6 leading-relaxed break-keep text-sm md:text-base">
                  <p>{service.description}</p>
                  <p className="mt-3 text-[#0F4C81] font-semibold">
                    자세한 상담을 통해 <br />
                    의뢰인의 특수한 상황에 맞는 <br className="hidden md:block" />
                    맞춤형 전략을 수립해 드립니다.
                  </p>
                </div>
                <ul className="space-y-2 mb-8">
                  <li className="flex items-center gap-2 text-xs md:text-sm text-[#0F4C81] font-medium">
        ...
        {/* 해결 프로세스 - 중앙 정렬 최적화 */}
        <div className="mt-20 md:mt-24 bg-[#F8F9FA] rounded-3xl p-6 md:p-12 border border-gray-100">
          <h2 className="text-xl md:text-3xl font-bold font-serif text-center mb-12 md:mb-16 tracking-tight break-keep">해결 프로세스</h2>
          <div className="flex flex-col md:flex-row justify-center items-center md:items-start gap-10 md:gap-12 relative max-w-6xl mx-auto">
            {[
              ...
              <div key={p.step} className="flex-1 flex flex-col items-center text-center relative z-10 group w-full max-w-[280px]">
                ...
                <p className="text-gray-500 text-[11px] md:text-sm leading-relaxed break-keep">{p.desc}</p>
                
                {idx < 3 && (
                  <div className="hidden md:block absolute top-8 left-[60%] w-[80%] h-[1px] bg-gradient-to-r from-[#0F4C81]/30 to-transparent -z-10" />
                )}
                {idx < 3 && (
                  <div className="md:hidden h-6 w-[1px] bg-gray-200 mt-4 -mb-6" />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="mt-20 md:mt-24 text-center">
          ...
          <div className="relative rounded-2xl overflow-hidden max-w-5xl mx-auto shadow-xl group">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-center justify-center p-4 md:p-12 text-center">
