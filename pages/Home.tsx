...
const Home: React.FC = () => {
  ...
  return (
    <div className="overflow-hidden">
      {/* Hero Section */}
      <section className="relative min-h-[60vh] md:min-h-[70vh] flex items-center bg-gradient-to-br from-[#F4F7FA] to-white py-12 md:py-24 overflow-hidden">
        <div className="container mx-auto px-4 md:px-6 relative z-10 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
            {/* Left Content */}
            <div className="lg:col-span-7 xl:col-span-8 text-center lg:text-left flex flex-col items-center lg:items-start">
              <span className="inline-block bg-[#002C5F] text-white px-3 py-1 rounded-full text-[10px] md:text-sm font-bold mb-4 md:mb-6 shadow-sm tracking-tight">
                행정 114의 핵심 철학
              </span>
              ...
              <p className="text-base md:text-2xl text-gray-700 font-bold mb-3 md:mb-4 break-keep leading-relaxed tracking-tight">
                <span className="text-[#F5A623]">'진실 규명'과 '법의 허점 파악'</span>을 통해<br className="hidden md:inline lg:hidden" /> 역전가능성을 증명합니다.
              </p>
              <p className="text-xs md:text-lg text-gray-500 font-medium break-keep leading-relaxed mb-8 md:mb-10 opacity-90 max-w-2xl">
              ...
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full sm:w-auto justify-center lg:justify-start">
                <Link to="/philosophy" className="bg-[#002C5F] text-white px-6 md:px-8 py-3.5 md:py-4 rounded-2xl font-bold text-sm md:text-lg flex items-center justify-center hover:bg-[#001a3a] transition-all shadow-lg active:scale-95">
                  투 트랙 솔루션 확인 <ChevronRight className="ml-1 w-4 h-4 md:w-5 md:h-5" />
                </Link>
                <Link to="/contact" className="bg-[#F5A623] text-white px-6 md:px-8 py-3.5 md:py-4 rounded-2xl font-bold text-sm md:text-lg text-center hover:bg-[#e09512] transition-all shadow-lg active:scale-95">
                  전략 상담 시작
                </Link>
              </div>
            </div>

            {/* Right Content: Small Premium Experts Card */}
            <div className="lg:col-span-5 xl:col-span-4 w-full max-w-md mx-auto">
              <div className="bg-white p-5 md:p-6 rounded-[24px] shadow-xl border border-gray-100 flex flex-col gap-4">
                <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                  <div className="flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-[#F5A623] animate-pulse"></span>
                    <span className="text-xs md:text-sm font-black text-[#002C5F]">분야별 핵심 책임 전문가</span>
                  </div>
                  <Link to="/experts" className="text-[10px] md:text-xs text-gray-400 font-bold hover:text-[#002C5F] transition-colors flex items-center gap-0.5">
                    자세히 보기 <ChevronRight className="w-3 h-3" />
                  </Link>
                </div>

                {/* Team Lead 1: 이경석 변호사 */}
                <div className="flex items-center gap-4 bg-[#F8F9FA] p-3 rounded-2xl border border-gray-50 hover:border-gray-200 transition-all">
                  <div className="w-16 h-16 md:w-20 md:h-20 shrink-0 bg-gray-200 rounded-xl overflow-hidden border border-white shadow-sm">
                    <img src={IMAGES.leeKyungSeok} alt="이경석 변호사" className="w-full h-full object-cover object-top" />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className="font-black text-sm md:text-lg text-[#002C5F] truncate">이경석</span>
                      <span className="text-[9px] md:text-[10px] bg-[#002C5F]/10 text-[#002C5F] px-1.5 py-0.5 rounded font-black">변호사</span>
                    </div>
                    <p className="text-[10px] md:text-xs text-gray-500 font-bold truncate leading-tight">행정·형사 소송 전략 설계</p>
                    <p className="text-[8px] md:text-[10px] text-[#F5A623] mt-1 font-black leading-none bg-[#F5A623]/10 px-1.5 py-0.5 rounded-full inline-block">법리·전략 설계 / 행정형사센터장</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 bg-[#F8F9FA] p-3 rounded-2xl border border-gray-50 hover:border-gray-200 transition-all">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className="font-black text-sm md:text-lg text-[#002C5F] truncate">김도현</span>
                      <span className="text-[9px] md:text-[10px] bg-[#F5A623]/10 text-[#F5A623] px-1.5 py-0.5 rounded font-black">소장 / 행정사</span>
                    </div>
                    <p className="text-[10px] md:text-xs text-gray-500 font-bold truncate leading-tight">공직 30년 현장 행정 실무</p>
                    <p className="text-[8px] md:text-[10px] text-gray-600 mt-1 font-black leading-none bg-gray-200/50 px-1.5 py-0.5 rounded-full inline-block">행정 실전 해법 전문가</p>
                  </div>
                </div>
              </div>
            </div>
